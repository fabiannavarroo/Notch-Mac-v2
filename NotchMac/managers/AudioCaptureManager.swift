...
final class AudioCaptureManager: ObservableObject {
    ...
    private let fftQueue = DispatchQueue(label: "com.fabiannavarrofonte.notchmac.audiocapture.fft", qos: .userInitiated)
    private let ioQueue = DispatchQueue(label: "com.fabiannavarrofonte.notchmac.audiocapture.io", qos: .userInteractive)
    private let lifecycleQueue = DispatchQueue(label: "com.fabiannavarrofonte.notchmac.audiocapture.lifecycle", qos: .userInitiated)
    ...
    private func startCaptureOnLifecycleQueue(pids: [pid_t]) {
        ...
        let aggregateUID = "com.fabiannavarrofonte.notchmac.audiotap.\(UUID().uuidString)"
        let aggregateDescription: [String: Any] = [
            kAudioAggregateDeviceNameKey: "NotchMacTapAggregate",
    ...
    private func performPermissionProbeOnLifecycleQueue() -> Bool {
        ...
        tapDescription.name = "NotchMac Audio Permission Probe"
        tapDescription.muteBehavior = .unmuted
        tapDescription.isPrivate = true
        ...
        let aggregateUID = "com.fabiannavarrofonte.notchmac.permissionprobe.\(UUID().uuidString)"
        let aggregateDescription: [String: Any] = [
            kAudioAggregateDeviceNameKey: "NotchMac Audio Permission Probe",
            kAudioAggregateDeviceUIDKey: aggregateUID,
            kAudioAggregateDeviceMainSubDeviceKey: "",
            kAudioAggregateDeviceIsPrivateKey: 1,
            kAudioAggregateDeviceIsStackedKey: 0,
